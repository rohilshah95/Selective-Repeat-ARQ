...
def main():
    ...
    for N_val in new_N:
        for i in range(5):
            ...
            startTime = time.time()
            ackReceiver = receiver(sys.argv[1], sys.argv[2],sys.argv[3],N_val,sys.argv[5],cliSocket)                    #Thread that receives ACKs from the Server
            fileHandler = fileReader(sys.argv[1], sys.argv[2],sys.argv[3],N_val,sys.argv[5],cliSocket, ackReceiver)     #Thread that reads the file and sending of packets
            fileHandler.join()
            ackReceiver.join()
            endTime = time.time()
            cliSocket.close()
    ...
    for MSS_val in new_MSS:
        for i in range(5):
            ...
            cliSocket.bind(('',cliPort))
            startTime = time.time()
            ackReceiver = receiver(sys.argv[1], sys.argv[2],sys.argv[3],N_val,MSS_val,cliSocket)                    #Thread that receives ACKs from the Server
            fileHandler = fileReader(sys.argv[1], sys.argv[2],sys.argv[3],N_val,MSS_val,cliSocket, ackReceiver)     #Thread that reads the file and sending of packets
            fileHandler.join()
            ackReceiver.join()
            endTime = time.time()
            cliSocket.close()
